...
def classifier(tmpdir):
    ...
    return SparseNaiveBayesClassifier(
        config=SparseNaiveBayesClassifier.get_default_config(),
...
def test_predictions_added(classifier, featurizer, tokenizer):
    ...
    # First we add tokens.
    tokenizer.process(training_data.training_examples)
